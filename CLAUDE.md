...
- **종결어미 다양화**: 같은 종결어미 반복 금지
  - "~더라" 반복 ❌ → "~았다/었다", "~는 것 같다", "~게 됐다", "~니까" 등으로 다양화
  - 예시:
    - ❌ "각각 쓰임새가 다르더라" → ✅ "각각 쓰는 타이밍이 달랐다"
    - ❌ "스킬이 고급 버전이더라" → ✅ "스킬이 고급 버전 같았다"
    - ❌ "헷갈리지 않더라" → ✅ "명확해졌다"
- **AI가 쓴 것처럼 보이지 않게**:
...
- [ ] **자연스러운 경험담으로 풀어쓰기**
  ...
  - 다양한 종결어미 사용: "~했다", "~게 됐다", "~니까", "~는 것 같다" 등
  - 같은 종결어미 반복 금지 (특히 "~더라" 남발 주의)
  - 괄호 남발 지양
